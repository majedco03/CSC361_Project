...
class SudokuGUI:
    def __init__(self, root, on_back=None):
        self.root = root
        self.on_back = on_back # Callback function to return to menu
        
        # We assume root is already fullscreen from GameHub, 
        # but if run standalone, we can set it.
    ...
    def setup_ui(self):
        # We create a main_frame to hold everything. 
        # This allows GameHub to track and destroy this specific frame.
        ...
        title_label.pack(side=tk.TOP, anchor="center") # Centered
        ...
        style.map("Dark.TCombobox", fieldbackground=[("readonly", "#102036")], background=[("active", "#203354")], foreground=[("readonly", "white")])
        ...
        ttk.Combobox(top_controls, textvariable=self.diff_var, values=["Easy", "Medium", "Hard", "Expert", "Extreme"], state="readonly", width=10, font=("Helvetica", 11), style="Dark.TCombobox").pack(side=tk.LEFT, padx=10)
        ...
        ttk.Combobox(algo_frame, textvariable=self.algo_var, values=["Smart CSP (MRV)", "Naive Backtracking", "Simulated Annealing (Numpy)"], state="readonly", width=25, font=("Helvetica", 11), style="Dark.TCombobox").pack(side=tk.LEFT, padx=10)
    ...
    def go_back(self):
        # Destroy the sudoku frame and call the menu callback
        self.main_frame.destroy()
        self.status_label.destroy() # Remove the status bar too
        if self.on_back:
            self.on_back()
        ...
        for r in range(9):
            for c in range(9):
                ...
                if (r, c) in self.generated_cells:
                    ...
                    cell.config(state="normal", bg="#ffffff", fg="#000000")
    ...
    def generate_new(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Error while generating puzzle:\n{e}")
    ...
    def solve_puzzle(self):
        for r in range(9):
            for c in range(9):
                ...
                if val.isdigit() and int(val) in range(1, 10): self.game.board[r][c] = int(val)
                else: self.game.board[r][c] = 0

        algo_choice = self.algo_var.get()
        self.status_label.config(text=f"Solving using {algo_choice}...")
        self.root.update()
        start_time = time.perf_counter()
        if "Smart" in algo_choice: solver = self.game.solve_csp_generator()
        elif "Naive" in algo_choice: solver = self.game.solve_naive_generator()
        else: solver = self.game.solve_annealing_generator()
        ...
        except StopIteration as e: solved = e.value
        ...
        if solved or "Annealing" in algo_choice:
            if "Annealing" not in algo_choice:
                for r in range(9):
                    for c in range(9):
                        if (r, c) not in self.generated_cells: self.cells[(r, c)].config(bg="#C8E6C9", fg="#2E7D32")
            else:
                self.update_gui_from_board()
                for r in range(9):
                    for c in range(9):
            ...
            messagebox.showwarning("Result", "Solver stopped or could not find a solution.")
